...
#            shapelock.release()
    ...
    def filter_jrk(self, idx, years_req=[]):
        ...
        for i in range(len(a)):
            ...
            y = np.array([i[1] for i in xy])#a[i][2]])
    ...
    def get_dataframe(self, transect, years_requested): 
        ...
        for idx in np.nonzero(idxs)[0]:
            ...
            for indx, yr in enumerate(years_included):
                # print(indx, yr, [years_requested.index(yr)])
                df_jrk = pd.DataFrame()
                x = x_values[indx]
                y = y_values[indx]
                ...
                df_jrk_yrs = pd.concat([df_jrk_yrs, df_jrk],axis=0) # df_jrk_yrs = df_jrk_yrs.append(df_jrk)
            df_jrk_all = pd.concat([df_jrk_all, df_jrk_yrs], axis=0)            # df_jrk_all = df_jrk_all.append(df_jrk_yrs)
